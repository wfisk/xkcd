...
define(['cmx/XKCD'], function(XKCD) {
  ...
  const ξ = function(thing, fn) {
    ...
    return function() {
      let val = thing;
      while (_.isFunction(val)) { val = val(); }
      if (_.isEqual(val, lastVal)) { return val; }
  ...
  var render = function(Δroot, back, Δbefore) {
    ...
    if (!this.updaters) { this.updaters = []; }
    ...
    let Δel = Δroot;
    if (Δbefore) { Δel = Δel.insert("g", Δbefore); }
    Δel = Δel.append("g");
    if (this["class"]) { Δel.attr("class", this["class"]); }
    if (this["data"]) { Δel.property("cmx", this["data"]); }
    dynamic(this["t"], function(val) { if (val) { return Δel.attr("transform", val); } });
    ...
    if (this.type === "text") {
      ...
      if (back) {
        ...
        dynamic(this["stroke-width"], function(val) { if (val) { return Δtext.style("stroke-width", val); } });
        dynamic(this["bgcolor"], function(val) { if (val) { return Δtext.style("stroke", val); } });
        ...
        if (this["border"]) {
          ...
          this.updaters.push(() => Array.from(polyline.updaters).map((updater) =>
            updater()));
        }
    ...
    const xkcdInterpolator = pts => {
      ...
      if (this["fill"] || this.closed) { res += "Z"; }
      return res;
    };
    ...
    dynamic(this["fill"], function(val) { if (val) { return Δpath.style("fill", val); } });
    if (back) {
      dynamic(this["back-stroke-width"], function(val) { if (val) { return Δpath.style("stroke-width", val); } });
      dynamic(this["back-stroke"], function(val) { if (val) { return Δpath.style("stroke", val); } });
      dynamic(this["points"], val => Δpath.attr("d", line.interpolate(backInterpolator)(val)));
    } else {
      dynamic(this["stroke-width"], function(val) { if (val) { return Δpath.style("stroke-width", val); } });
      dynamic(this["stroke"], function(val) { if (val) { return Δpath.style("stroke", val); } });
      dynamic(this["points"], val => Δpath.attr("d", line.interpolate(xkcdInterpolator)(val)));
    }
  ...
  return Renderer = class Renderer {
    ...
    constructor(root, width, height, marginX, marginY) {
      ...
      this.Δel = this.Δsvg.append("g").attr("transform", "translate(" + this.marginX + ", " + this.marginY + ")") // implement margin
    ...
    draw(Δelement) {
      let item;
      if (!Δelement) { Δelement = this.Δlayers.append("g").attr("class", "static"); }
      ...
      for (item of Array.from(this.calls)) {
        item.update = function() {
          ...
          return Array.from(this.updaters).map((update) =>
            update());
        };
      }
    ...
    circle(radius, opts) {
      ...
      const angle = d3.scale.linear().domain([0, N - 1]).range([0, R]);
      const l = d3.svg.line.radial().interpolate("basis").tension(0).radius(radius).angle((d, i) => angle(i));
    ...
    path(spec, opts) {
      ...
      const lazyEvaluation = () => {
        ...
        while (i < (points.length-1)) {
          ...
          if (!flat(left, mid, right)) { points2.push(mid); }
          i++;
        }
        ...
        if (points2.length>=3) {
          if (flat(points2[points2.length-3], points2[points2.length-2], points2[points2.length-1])) { points2.splice(points2.length-2, 1); }
        }
